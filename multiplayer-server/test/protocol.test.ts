...
import { BINARY_HEADER_BYTES, BINARY_MAGIC, MAX_MESSAGE_BYTES, isRoomCode, makeRoomCode, parseBinaryHeader, parseEnvelope } from "../src/protocol";
...
describe("binary gameplay packets", () => {
  it("reads the explicit little-endian header", () => {
    ...
    view.setUint32(0, BINARY_MAGIC, true); view.setUint16(4, 1, true); view.setUint8(6, 1); view.setUint8(7, 2);
  ...
  it("rejects truncated and inconsistent payloads", () => {
    ...
    const view = new DataView(packet); view.setUint32(0, BINARY_MAGIC, true); view.setUint16(4, 1, true); view.setUint8(6, 1); view.setUint8(7, 0); view.setUint32(16, 99, true);
...
describe("wire envelopes", () => {
  it("accepts the current protocol", () => {
    expect(parseEnvelope(JSON.stringify({ v: 1, type: "input", seq: 4, tick: 12, payload: { moveX: 1 } })))
      .toMatchObject({ v: 1, type: "input", seq: 4, tick: 12 });
  });

  it("rejects incompatible, malformed, and oversized messages", () => {
    expect(parseEnvelope("{")) .toBeNull();
    expect(parseEnvelope(JSON.stringify({ v: 2, type: "input", seq: 1, tick: 1 }))).toBeNull();
    expect(parseEnvelope(JSON.stringify({ v: 1, type: "unknown", seq: 1, tick: 1 }))).toBeNull();
