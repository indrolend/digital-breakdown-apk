...
public final class MainActivity extends Activity {
    ...
    private void showMainMenu(){page("READY");Button solo=button("SOLO");solo.setOnClickListener(v->{multiplayer.close();NativeBridge.startSolo();menuScroller.setVisibility(View.GONE);});menu.addView(solo);Button online=button("ONLINE");online.setOnClickListener(v->showOnline());menu.addView(online);Button settings=button("SETTINGS");settings.setOnClickListener(v->showSettings());menu.addView(settings);}
