...
void Game::updateSoulLattices() {
    ...
    for(int i=0;i<TARGET_COUNT;++i){TargetState& target=state_.targets[i];
        ...
        // Human targets never render the soul lattice. Every lifecycle entry
        // into or out of the soul role resets the lattice, so doing its full
        // 27-node spring simulation here only burns frame time.
        if(!target.slurpable&&target.soulMorph<=0.001f&&target.ingestProgress<=0.001f){
            target.tetherVisible=false;
            continue;
        }
