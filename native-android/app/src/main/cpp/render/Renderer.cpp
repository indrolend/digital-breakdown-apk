...
void Renderer::drawRoomTile(const float* viewProj, const GameState& state, int tileIndex) {
    ...
    if(state.slopeLab)for(int i=0;i<state.rockSupportCount;++i){const auto& support=state.rockSupports[i];const VisualColor substrate=roomSubstrateColor(plan.setting);const float rock[4]={substrate.r*0.82f,substrate.g*0.82f,substrate.b*0.82f,1};drawFacetedRock(viewProj,support.prop,support.roomSeed,support.roomIndex,support.propIndex,z0,rock);}
    ...
    for(int i=0;i<early_browser_visuals::environmentPropCount(plan);++i){
        ...
        const float structure[4]={0.40f,0.47f,0.50f,1},roof[4]={0.28f,0.34f,0.38f,1},dark[4]={0.05f,0.08f,0.09f,1},lawn[4]={0.20f,0.39f,0.23f,1},marker[4]={0.48f,0.55f,0.58f,1},cap[4]={0.72f,0.90f,0.94f,1};
        if(prop.primitive==EnvironmentPrimitive::House){for(const auto& part:house_geometry::parts(prop,z0)){const float* color=part.surface==0?structure:(part.surface==3?dark:roof);drawBox(viewProj,part.center,part.size,part.yaw,color);}}
        else if(prop.primitive==EnvironmentPrimitive::Tree){
            int trunkIndex=0;for(const auto& part:tree_geometry::trunkParts(prop,z0)){const float shade=static_cast<float>(trunkIndex++);const float trunkPart[4]={0.25f+shade*0.02f,0.20f+shade*0.01f,0.14f,1};drawBox(viewProj,part.center,part.size,part.yaw,trunkPart);}
            bool activeClimb=false;
            if(state.player.treeClimbing&&tileIndex==state.topology.currentTileIndex&&state.player.treeCollider>=0&&state.player.treeCollider<state.debug.colliderCount){const RoomCollider& climbTree=state.roomColliders[state.player.treeCollider];activeClimb=climbTree.kind==RoomColliderKind::TreeTrunk&&std::abs(climbTree.center.x-prop.center.x)<0.05f&&std::abs(climbTree.center.z-prop.center.z)<0.05f;}
            for(const auto& part:tree_geometry::crownParts(prop,z0)){const float radius=std::max(part.size.x,part.size.z)*0.58f;const bool occluding=activeClimb&&early_browser_visuals::treeFoliageClusterOccludes(state.camera.pos,state.player.pos,part.center,radius);const float crown[4]={0.16f,0.43f,0.23f,occluding?0.28f:1.0f};if(occluding){glEnable(GL_BLEND);glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);glDepthMask(GL_FALSE);}drawBox(viewProj,part.center,part.size,part.yaw,crown);if(occluding){glDepthMask(GL_TRUE);glDisable(GL_BLEND);}}
        }
