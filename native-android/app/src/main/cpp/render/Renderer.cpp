...
void Renderer::drawRoomTile(const float* viewProj, const GameState& state, int tileIndex) {
    ...
    for(int i=0;i<state.rockSupportCount;++i){const auto& support=state.rockSupports[i];const VisualColor substrate=roomSubstrateColor(plan.setting);const float rock[4]={substrate.r*0.82f,substrate.g*0.82f,substrate.b*0.82f,1};drawFacetedRock(viewProj,support.prop,support.roomSeed,support.roomIndex,support.propIndex,z0,rock);}
