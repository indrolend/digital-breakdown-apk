...
int runRoomInspectorSmoke(Game& game){
    ...
    for(int premiseIndex=0;premiseIndex<static_cast<int>(early_browser_visuals::RoomPremise::Count);++premiseIndex){
        for(int sample=0;sample<3;++sample){
            ...
            std::printf("ROOM_INSPECT premise=%s playstyle=%s seed=%d room=%d route=%s band=%s surfaces=%d edges=%d colliders=%d props=%d enemies=%d/%d playable=%s\n",early_browser_visuals::premiseName(r.premise),early_browser_visuals::playstyleName(r.playstyle),r.seed,r.roomIndex,r.requiredRouteValid?"VALID":"INVALID",gameplay::traversalDifficultyName(r.requiredBand),r.traversalSurfaceCount,r.traversalEdgeCount,r.colliderCount,r.presentationPropCount,r.enemyCount,r.enemyBudget,playable?"YES":"NO");
