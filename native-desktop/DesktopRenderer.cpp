...
void DesktopRenderer::drawHumanModel(const TargetState& target,float time,early_browser_visuals::RoomSetting setting,bool shadow,float perceptionHeadYaw,float perceptionHeadPitch) const {
    ...
    glPushMatrix();glTranslatef(root.x,root.y,root.z);glMultMatrixf(matrix);glScalef(pose.scale*visual.expressiveScale.x,pose.scale*(1.0f-visual.bodyCompression)*visual.expressiveScale.y,pose.scale*visual.expressiveScale.z);if(shadow)glColor4f(0.012f,0.018f,0.022f,0.28f);else gradedColor(cueColor[0],cueColor[1],cueColor[2],cueColor[3]);glBegin(GL_TRIANGLES);
