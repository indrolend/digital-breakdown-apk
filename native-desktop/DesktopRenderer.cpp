...
float phoneTextWidth(const std::string& value,float scale){return static_cast<float>(value.size())*6.0f*scale;}

void phoneScreenText(const PhoneTransformState& phone,const std::string& value,float x,float y,float scale,float r,float g,float b,float a){
    float pen=x;
    for(char c:value){
...
void phoneScreenPaletteText(const PhoneTransformState& phone,const std::string& value,float x,float y,float scale,float time,float a){
    ...
    for(std::size_t i=0;i<value.size();++i){
        ...
        phoneScreenText(phone,std::string(1,c),pen,y+std::sin(time*1.45f+static_cast<float>(i)*0.42f)*scale*0.18f,scale,0.55f+color.x*0.42f,0.65f+color.y*0.34f,0.72f+color.z*0.28f,a);
        pen+=6.0f*scale;
    }
}
...
void DesktopRenderer::drawHud(const GameState& state) const {
    ...
    if(state.cinematic.introActive){
        ...
        const float panelW=std::min(340.0f,menuCanvasW*0.58f),panelH=470.0f,panelX=(menuCanvasW-panelW)*0.5f,panelY=(menuCanvasH-panelH)*0.5f;
        rotatedQuad(panelX+panelW*0.5f,panelY+panelH*0.5f,panelW+34.0f,panelH+46.0f,0,0.004f,0.010f,0.014f,0.58f*alpha);
        rotatedQuad(panelX+panelW*0.5f,panelY+panelH*0.5f,panelW,panelH,0,0.018f,0.046f,0.054f,0.68f*alpha);
        quad(panelX+20.0f,panelY+20.0f,panelW-40.0f,1.0f,0.62f,0.96f,1.0f,0.40f*alpha);
        quad(panelX+20.0f,panelY+panelH-20.0f,panelW-40.0f,1.0f,0.62f,0.96f,1.0f,0.24f*alpha);
        const std::string status="DATA";const float statusScale=3.0f,statusW=status.size()*6*statusScale;
        floatingPaletteText(status,panelX+(panelW-statusW)*0.5f,panelY+118,statusScale,0.96f*alpha);
        const std::string start="START";const float startScale=2.0f,startW=start.size()*6*startScale;
        floatingText(start,panelX+(panelW-startW)*0.5f,panelY+212,startScale,0.75f,0.96f,1.0f,alpha);
    ...
    if(!state.started||pausedSolo) {
        ...
        const float panelW=std::min(controlsPage?390.0f:340.0f,menuCanvasW*0.72f);
        const float panelH=controlsPage?560.0f:470.0f;
        ...
        rotatedQuad(panelX+panelW*0.5f,panelY+panelH*0.5f,panelW+34.0f,panelH+46.0f,0,0.004f,0.010f,0.014f,0.62f*menuAlpha);
        rotatedQuad(panelX+panelW*0.5f,panelY+panelH*0.5f,panelW,panelH,0,0.014f,0.040f,0.048f,0.72f*menuAlpha);
        quad(panelX+20.0f,panelY+20.0f,panelW-40.0f,1.0f,0.62f,0.96f,1.0f,0.42f*menuAlpha);
        quad(panelX+20.0f,panelY+panelH-20.0f,panelW-40.0f,1.0f,0.62f,0.96f,1.0f,0.25f*menuAlpha);
