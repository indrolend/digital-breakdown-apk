...
console.log("[STYLO-V2.2-GAMEPAD] boot");
...
function updateGamepadDiagnostics() {
  ...
  gamepadDiag.lx = gpAxis(pad.axes[0], 0.18);
  gamepadDiag.ly = gpAxis(pad.axes[1], 0.18);
  gamepadDiag.rx = gpAxis(pad.axes[2], 0.18);
  gamepadDiag.ry = gpAxis(pad.axes[3], 0.18);
  ...
  if (now - gamepadDiag.lastLogTime > 1500) {
    console.log("[STYLO-V2.2-GAMEPAD] pad", gamepadDiag.id);
    gamepadDiag.lastLogTime = now;
  }

  if (
    Math.abs(gamepadDiag.lx) > 0 ||
    Math.abs(gamepadDiag.ly) > 0 ||
    Math.abs(gamepadDiag.rx) > 0 ||
    Math.abs(gamepadDiag.ry) > 0
  ) {
    input.last = "pad:axis";
  }

  if (gamepadDiag.a) input.last = "pad:A";
  if (gamepadDiag.x) input.last = "pad:X";
  if (gamepadDiag.lt || gamepadDiag.rt) input.last = "pad:trigger";
}
...
function setCameraMode(mode) {
  ...
  console.log("[STYLO-V2.2-GAMEPAD] camera mode", mode, cameraDebug.modeLabel);
}
...
function makeHud() {
  ...
  hud.textContent = "STYLO V2.2 GAMEPAD";
...
function resetRoom() {
  ...
  resetTargets();
  resetCaptures();
  console.log("[STYLO-V2.2-GAMEPAD] room reset", roomIndex);
}
...
function updateCaptures(dt) {
  ...
  if (!roomClear && filled >= captures.length) {
    ...
    console.log("[STYLO-V2.2-GAMEPAD] room clear");
...
function updateCamera(dt) {
  ...
  if (input.lookX !== 0 || input.lookY !== 0) {
    ...
    input.last = "look:" + input.lookX.toFixed(1) + "," + input.lookY.toFixed(1);
  }
...
function updateHud() {
  ...
  hud.textContent =
    "DIGITAL BREAKDOWN / STYLO V2.2 GAMEPAD.2 GAMEPAD\n" +
    "FPS " + fps + "\n" +
    ...
    "LXLY " + gamepadDiag.lx.toFixed(2) + "," + gamepadDiag.ly.toFixed(2) + "  RXRY " + gamepadDiag.rx.toFixed(2) + "," + gamepadDiag.ry.toFixed(2) + "\n" +
    ...
    "CAM YAW " + cameraRig.yaw.toFixed(2) + " PITCH " + cameraRig.pitch.toFixed(2) + "\n" +
    "MANUAL " + cameraRig.manualTimer.toFixed(2) + " " + cameraDebug.lastManual + "\n" +
    "LOOK " + input.lookX.toFixed(1) + "," + input.lookY.toFixed(1) + "\n" +
    "WASD MOVE  JLIK CAM  1/2/3/4 CAM MODE  Q VACUUM";
}
...
function boot() {
  ...
  console.log("[STYLO-V2.2-GAMEPAD] READY");
  requestAnimationFrame(loop);
  console.log("[STYLO-V2.2-GAMEPAD] RUNNING");
}
