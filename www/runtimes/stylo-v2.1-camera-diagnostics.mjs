...
console.log("[STYLO-V2.1-CAMERA] boot");
...
function setCameraMode(mode) {
  ...
  console.log("[STYLO-V2.1-CAMERA] camera mode", mode, cameraDebug.modeLabel);
}
...
function resetRoom() {
  ...
  resetTargets();
  resetCaptures();
  console.log("[STYLO-V2.1-CAMERA] room reset", roomIndex);
}
...
function updateCaptures(dt) {
  ...
  if (!roomClear && filled >= captures.length) {
    ...
    console.log("[STYLO-V2.1-CAMERA] room clear");
...
function updateHud() {
  ...
  hud.textContent =
    "DIGITAL BREAKDOWN / STYLO V2.1 CAMERA\n" +
    "FPS " + fps + "\n" +
...
function boot() {
  ...
  console.log("[STYLO-V2.1-CAMERA] READY");
  requestAnimationFrame(loop);
  console.log("[STYLO-V2.1-CAMERA] RUNNING");
}
