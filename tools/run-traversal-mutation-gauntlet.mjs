...
const mutations = [
  ...
  { id: 'M05', file: 'native/game/Game.cpp', critical: true, ownership: 'CHECK_OWNED', description: 'enemy traversal speed reduced to near-stall', from: 'constexpr float HUMAN_WALK_SPEED = 0.72f;', to: 'constexpr float HUMAN_WALK_SPEED = 0.08f;' },
...
console.log(`MUTATION_GAUNTLET=${packet.restored ? 'PASS' : 'FAIL'} variants=${results.length} durationMs=${packet.totalExperimentMs} evidence=${outputPath}`);
if (!packet.restored || results.some(result => result.classification === 'INVALID_MUTATION' || (result.critical && result.classification === 'MISSED'))) process.exitCode = 1;
