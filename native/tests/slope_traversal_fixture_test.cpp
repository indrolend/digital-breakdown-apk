...
int main(){
    ...
    const auto flatAuthority=ascent.debugPlayerSupportAt(0.0f,13.5f),slopeAuthority=ascent.debugPlayerSupportAt(0.0f,7.0f),plateauAuthority=ascent.debugPlayerSupportAt(0.0f,0.0f);
    if(flatAuthority.source!=SupportSource::Ground||slopeAuthority.source!=SupportSource::Slope||slopeAuthority.sourceIndex!=0||plateauAuthority.source!=SupportSource::Collider||plateauAuthority.sourceIndex!=0){std::fprintf(stderr,"WORLD_CONTACT_FAIL source authority ground=%d slope=%d plateau=%d\n",static_cast<int>(flatAuthority.source),static_cast<int>(slopeAuthority.source),static_cast<int>(plateauAuthority.source));return 1;}
    ...
    if(!centerSurface.inside||!near(centerGameplay.height,centerSurface.height+0.08f)||centerGameplay.normal.y<faceted_rock::WalkableNormalY||centerGameplay.source!=SupportSource::RockFacet||centerGameplay.sourceIndex!=0){std::fprintf(stderr,"ROCK_SUPPORT_FAIL shared facet support\n");return 1;}
