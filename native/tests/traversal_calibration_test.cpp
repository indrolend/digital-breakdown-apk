...
int main(){
    ...
    for(int premiseIndex=0;premiseIndex<static_cast<int>(early_browser_visuals::RoomPremise::Count);++premiseIndex){
        ...
        if(!state.roomInspector||state.roomInspectorPremise!=premise||!early_browser_visuals::matchesInspectorPremise(plan,premise)||!state.roomClear||state.requiredSouls!=0||!report.seedSelectionValid||
           ...
           report.colliderCount!=state.debug.colliderCount||report.colliderCount+state.slopeSupportCount+state.rockSupportCount!=expectedColliders||report.presentationPropCount!=expectedProps||report.enemyCount!=0){
        ...
        if(physicalSurfaces){
            ...
            const auto& surface=plan.traversal.surfaces[plan.traversal.surfaceCount-1];const auto expected=early_browser_visuals::physicalTraversalObstacle(surface);bool materialized=false;
            if(early_browser_visuals::usesShallowElevation(plan,surface))for(int i=0;i<state.slopeSupportCount;++i){const auto& slope=state.slopeSupports[i];materialized|=slope.minX==expected.center.x-expected.size.x*0.5f&&slope.maxX==expected.center.x+expected.size.x*0.5f&&slope.minZ==expected.center.z-expected.size.z*0.5f&&slope.maxZ==expected.center.z+expected.size.z*0.5f&&slope.highHeight==expected.size.y;}
            else for(int i=0;i<state.debug.colliderCount;++i){const auto& collider=state.roomColliders[i];materialized|=collider.bottomY==0.0f&&collider.topY==expected.size.y&&collider.center.x==expected.center.x&&collider.center.y==expected.center.y&&collider.center.z==expected.center.z&&collider.width==expected.size.x&&collider.height==expected.size.y&&collider.depth==expected.size.z;}
            if((plan.traversalIntent!=early_browser_visuals::RoomTraversalIntent::Playground&&plan.traversalIntent!=early_browser_visuals::RoomTraversalIntent::Funnel)||surface.required||!materialized){std::fprintf(stderr,"TRAVERSAL_CALIBRATION_FAIL optional traversal surface was not materialized as one ground-supported shape\n");return 1;}
        }
